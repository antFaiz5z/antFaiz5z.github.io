...
ChildA["子协程 A（失败）"] --> Cancel["CancellationException"]
Cancel --> Parent["父协程"]
Parent --> All["全部取消"]
All --> ChildB["子协程 B 也被取消<br/>（即使 B 没问题）"]
{% endmermaid %}

### SupervisorJob 的异常传播

{% mermaid flowchart TB %}
ChildAFail["子协程 A（失败）"] --> Cancel2["CancellationException"]
Cancel2 --> Parent2["父协程"]
Parent2 --> SelfOnly["只取消 A 自己"]
Parent2 --> ChildBRun["子协程 B 继续运行<br/>（不受影响）"]
{% endmermaid %}
